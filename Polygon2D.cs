...
namespace Polyhedra2DZone {
    ...
    public class Polygon2D : MonoBehaviour, ILayer {
        ...
        #region Vertex
        public virtual int VertexCount { get { return normalizedVertices.Count; } }
        public virtual Vector2 GetVertex(int i) {
            validator.CheckValidation();
            return layerVertices[i];
        }
        public virtual void SetVertex(int i, Vector2 value) {
            Invalidate();
            normalizedVertices[i] = LayerToLocal(value);
        }
        public virtual int AddVertex(Vector2 v) {
            Invalidate();
            var i = normalizedVertices.Count;
            normalizedVertices.Add(LayerToLocal(v));
            return i;
        }
